...
loop.run_until_complete(start_Dipesh())
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        Dipesh = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        if len(e.text) > 6:
            bc = Dipesh[0]
            text = "Joining..."
...
async def spam(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(Dipesh) == 2:
            message = str(Dipesh[1])
            counter = int(Dipesh[0])
            if counter > 100:
...
async def spam(e):
    ...
    if e.sender_id in SMEX_USERS:
        if e.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None)
        smex = await e.get_reply_message()
        Dipesh = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
        Dipeshsexy = Dipesh[1:]
        if len(Dipeshsexy) == 2:
            message = str(Dipeshsexy[1])
            counter = int(Dipeshsexy[0])
            sleeptime = float(Dipesh[0])
            for _ in range(counter):
